...
$types = array('weekly', 'ltc');
$type_descriptions = array(
    'weekly' => 'BTC Weekly Futures (USD)',
    'ltc' => 'LTC Weekly Futures (USD)',
    'mri' => '796Xchange-MRI',
    'asicminer' => 'ASICMINER-796',
    'rsm' => 'RSM',
);

foreach($types as $type) {
    $url = 'http://api.796.com/v3/futures/ticker.html?type=' . $type;
    $data = curl_get($url, $post);
    echo "<tr>";
    echo "<td>" . $type_descriptions[$type] . "- Ticker</td>";
    echo "<td><pre>" . print_r($post, TRUE) .  "</pre></td>";
    echo "<td><pre>" . print_r($data, TRUE) . "</pre></td>";
    echo "</tr>";
    
    $url = 'http://api.796.com/v3/futures/depth.html?type=' . $type;
    $data = curl_get($url, $post);
    echo "<tr>";
    echo "<td>" . $type_descriptions[$type] . "- Depth</td>";
    echo "<td><pre>" . print_r($post, TRUE) .  "</pre></td>";
    echo "<td><pre>" . print_r($data, TRUE) . "</pre></td>";
    echo "</tr>";
    
    $url = 'http://api.796.com/v3/futures/trades.html?type=' . $type;
    $data = curl_get($url, $post);
    echo "<tr>";
    echo "<td>" . $type_descriptions[$type] . "- Trades</td>";
    echo "<td><pre>" . print_r($post, TRUE) .  "</pre></td>";
    echo "<td><pre>" . print_r($data, TRUE) . "</pre></td>";
    echo "</tr>";
...
foreach($types as $type) {
    $url = ' http://api.796.com/v3/stock/ticker.html?type=' . $type;
    $data = curl_get($url, $post);
    echo "<tr>";
    echo "<td>" . $type_descriptions[$type] . "- Ticker</td>";
    echo "<td><pre>" . print_r($post, TRUE) .  "</pre></td>";
    ...
    $url = ' http://api.796.com/v3/stock/depth.html?type=' . $type;
    $data = curl_get($url, $post);
    echo "<tr>";
    echo "<td>" . $type_descriptions[$type] . "- Depth</td>";
    echo "<td><pre>" . print_r($post, TRUE) .  "</pre></td>";
    ...
    $url = 'http://api.796.com/v3/stock/trades.html?type=' . $type;
    $data = curl_get($url, $post);
    echo "<tr>";
    echo "<td>" . $type_descriptions[$type] . "- Trades</td>";
    echo "<td><pre>" . print_r($post, TRUE) .  "</pre></td>";
    ...
    $url = 'http://api.796.com/v3/stock/settle.html?type=' . $type;
    $data = curl_get($url, $post);
    echo "<tr>";
    echo "<td>" . $type_descriptions[$type] . "- Settlements</td>";
    echo "<td><pre>" . print_r($post, TRUE) .  "</pre></td>";
    echo "<td><pre>" . print_r($data, TRUE) . "</pre></td>";
    echo "</tr>";
}
...
$url  = 'https://796.com/v2/user/get_info';
$data = curl_get($url,$post );
echo "<tr>";
